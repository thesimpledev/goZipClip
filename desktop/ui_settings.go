...
type settingsForm struct {
	...
	clientID     *widget.Entry
	clientSecret *widget.Entry
...
func newSettingsForm() *settingsForm {
	return &settingsForm{
		...
		clientSecret: widget.NewPasswordEntry(),
		...
		automatic:    widget.NewCheck("Splice and delete without approval", nil),
...
func (f *settingsForm) fill(cfg Config) {
	...
	f.clientID.SetText(cfg.TwitchClientID)
	f.clientSecret.SetText(cfg.TwitchClientSecret)
	...
	f.automatic.SetChecked(cfg.AutomaticMode)
...
func (f *settingsForm) collect() (Config, error) {
	...
	cfg.TwitchClientID = strings.TrimSpace(f.clientID.Text)
	cfg.TwitchClientSecret = strings.TrimSpace(f.clientSecret.Text)
	...
	cfg.AutomaticMode = f.automatic.Checked
	var numErr error
...
func (u *UI) buildSettingsPane() fyne.CanvasObject {
	...
	form := widget.NewForm(
		...
		widget.NewFormItem("Twitch client ID", u.form.clientID),
		widget.NewFormItem("Twitch client secret", u.form.clientSecret),
		...
		widget.NewFormItem("Automatic mode", u.form.automatic),
